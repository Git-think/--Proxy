...
| 文件名 | 功能 | 描述 |
| --- | --- | --- |
| `set-env` | **设置/覆盖** 环境变量 | 用于修改或添加**任何**环境变量，例如 `SERVICE_PORT` 或 `LOG_LEVEL`。处理优先级最高，会覆盖 `.env` 文件中的同名变量。处理后文件会被自动删除。 |
| `add` | **增量添加** 数据 | 主要用于动态添加**账户** (`ACCOUNTS`)。处理后文件会被自动删除。 |
| `reload-env` | **增量重载** 环境变量 | 用于从 `.env` 文件中重新加载**账户** (`ACCOUNTS`) 或**代理** (`SOCKS5_PROXIES`)，主要用于增量添加新数据或重置代理状态。处理后文件会被自动删除。 |
